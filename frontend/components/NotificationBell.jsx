...
/**
 * NotificationItem — a single row in the notification panel.
 *
 * @param {object} props
 * @param {object} props.notification - Notification data object.
 * @param {string} props.notification.id - Unique notification identifier.
 * @param {string} props.notification.type - Escrow event type key from ESCROW_EVENT_TYPES.
 * @param {string} props.notification.title - Short notification title.
 * @param {string} [props.notification.message] - Optional longer description.
 * @param {string|Date} props.notification.createdAt - Creation timestamp.
 * @param {boolean} props.notification.read - Whether the notification has been read.
 * @param {function} props.onMarkAsRead - Callback invoked with the notification id.
 * @param {function} props.onDismiss - Callback invoked with the notification id.
 */
...
/**
 * NotificationBell — bell icon button that opens the notification panel.
 *
 * Renders an accessible bell trigger with an unread-count badge, an sr-only
 * live region that announces count changes to screen readers, and a dropdown
 * panel that lists all notifications. Every interactive element carries an
 * explicit aria-label so assistive technology can identify it without relying
 * on visible text alone.
 *
 * @param {object} props
  ...
  return (
    <div className={cn('relative', className)}>
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute -top-0.5 -right-0.5 flex h-4 min-w-[1rem] items-center justify-center rounded-full bg-red-600 px-1 text-[10px] font-semibold text-white" aria-hidden="true">
      ...
      {open && (
        ...
        >
          ...
          {notifications.length === 0 ? (
            <p className="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">You&apos;re all caught up.</p>
          ) : (
