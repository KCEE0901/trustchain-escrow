 ...
 *
 * Accessibility:
 *  - Card body is focusable (tabIndex=0) when an onClick handler is provided,
 *    and activates with Enter or Space.
 *  - Action buttons expose visible focus rings via focus-visible.
 *  - Cancel confirmation is a proper ARIA dialog with focus trapping and Escape
 *    dismissal; focus returns to the card body when dismissed.
 *  - Progress bar uses role="progressbar" with aria-valuenow / aria-valuemin /
 *    aria-valuemax / aria-label.
 *  - Status badge, icons, and chevron carry aria-hidden="true" so screen readers
 *    receive the information via the card's aria-label instead.
 *
 * @param {object}  props
 ...
 * @param {Function} [props.onPause]   — called with (id) when Pause is activated
 * @param {Function} [props.onResume]  — called with (id) when Resume is activated
 * @param {Function} [props.onCancel]  — called with (id) when Cancel is confirmed
...
/**
 * Inline confirmation dialog rendered when the user activates Cancel.
 * Dismissible with Escape; Tab is trapped inside until resolved;
 * focus returns to the card body on dismiss.
 */
...
export default function PaymentStreamCard({ stream, onPause, onResume, onCancel, onClick }) {
  const {
    id,
    recipient,
    totalAmount,
  ...
  const handleCardKeyDown = useCallback(
    (e) => {
      // Escape dismisses the cancel dialog when it is open
      if (e.key === 'Escape' && showCancelConfirm) {
        setShowCancelConfirm(false);
        return;
      }
      // Enter / Space activates the card click handler (only when focused on the card itself)
  ...
  const handleCancelDismiss = useCallback(() => {
    setShowCancelConfirm(false);
    // Return focus to card body so keyboard users don't lose their position
    cardRef.current?.focus();
  }, []);

  /**
   * Wraps a click handler so that button elements also respond to Enter and
   * Space — browsers fire click on Enter for <button> by default, but this
   * ensures Space also works consistently across all browsers.
   */
    ...
    >
      ...
      {(startDate || endDate) && (
        <div className="flex gap-4 mb-4 text-xs text-gray-500 dark:text-gray-400">
          {startDate && (
            <span>
              Start:{' '}
              <span className="text-gray-700 dark:text-gray-300">{formatDate(startDate)}</span>
            </span>
          )}
          {endDate && (
            <span>
              End:{' '}
              <span className="text-gray-700 dark:text-gray-300">{formatDate(endDate)}</span>
            </span>
          )}
        </div>
      )}
      ...
      {isInteractive && (
        ...
        >
          ...
          {/* Cancel — shows confirmation dialog when activated */}
      ...
      {showCancelConfirm && (
        <CancelConfirmDialog onConfirm={handleCancelConfirm} onDismiss={handleCancelDismiss} />
      )}
    </article>
  );
}
