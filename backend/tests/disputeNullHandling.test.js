 ...
 *
 * These tests verify that the `loadDispute` middleware uses strict equality
 * checks (`=== null`) throughout — not a mix of `== null`, `=== undefined`,
 * or truthy guards — and that every route requiring a resolved dispute object
 * gets a consistent 404 when the record does not exist, rather than a 500 from
 * a later handler trying to access a property on `undefined`.
 *
 * Key regression case: previously, routes like POST /:id/resolve/auto and
 * POST /:id/appeals called `req.dispute.id` without ever setting `req.dispute`,
 * causing a TypeError when the dispute was absent. Now `loadDispute` always
 * sets the value or short-circuits with a proper HTTP response.
 */
...
// Auth middleware — provide a user with ARBITRATOR role to reach deeper handlers
...
// MFA middleware — skip MFA for tests
...
// fileUpload — skip heavy middleware
...
// IPFS service — not needed for these tests
...
beforeEach(() => {
  ...
  // Prisma returns null (never undefined) for a missing record
...
describe('loadDispute middleware — null/undefined handling (Issue #122)', () => {
  it('returns 404 (not 500) when Prisma returns null for POST /:id/resolve/auto', async () => {
    // REGRESSION: previously req.dispute was undefined here, causing
    //   "TypeError: Cannot read properties of undefined (reading 'id')"
    //   in autoResolve. Now loadDispute returns 404 when dispute === null.
    prismaMock.dispute.findFirst.mockResolvedValue(null); // strict null from Prisma
    const app = buildApp();
...
// ── loadDispute validation: invalid id values ─────────────────────────────────
...
describe('loadDispute middleware — valid dispute flows', () => {
  ...
  it('does not call Prisma when id validation fails — no behavior change for valid inputs', async () => {
    prismaMock.dispute.findFirst.mockResolvedValue(mockDispute);
    prismaMock.disputeAppeal.findFirst.mockResolvedValue(null);
    const app = buildApp();

    // Valid id still reaches the DB as before
    const res = await request(app).post('/api/disputes/42/resolve/auto');
    // findFirst called exactly once (loadDispute) — not multiple times
