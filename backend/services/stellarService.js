...
const submitTransaction = async (signedXdr) => {
  return withSpan(
    ...
    async (span) => {
      // Deserialize the signed XDR before sending — catch bad input early
      let tx;
      try {
      ...
      // Submit to the Stellar RPC node
      let sendResult;
      try {
      ...
      if (sendResult.status === 'ERROR') {
        ...
        return {
          ...
          message:
            `Transaction rejected by the Stellar network (hash: ${sendResult.hash}). ` +
      ...
      return {
        ...
        status: 'TIMEOUT',
        message:
          `Transaction ${hash} was submitted but did not reach a final state after 30 polling ` +
          `attempts (60 s). It may still be included in a future ledger — check the Stellar ` +
          `network directly using the hash.`,
      };
    },
  );
};
